...
export default function AddPropertyModal({ isOpen, onClose, onSuccess }: AddPropertyModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const ownerFullName = formData.isCompany 
        ? formData.companyName 
        : `${formData.ownerFirstName} ${formData.ownerLastName}`.trim()

      const payload = {
        ownerFirstName: formData.isCompany ? null : formData.ownerFirstName,
        ownerLastName: formData.isCompany ? null : formData.ownerLastName,
        ownerFullName,
  ...
  const canProceed = () => {
    ...
    if (step === 2) {
      ...
      return formData.ownerFirstName.trim() !== '' || formData.ownerLastName.trim() !== ''
    }
    return true
  }
